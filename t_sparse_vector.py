...
class TestSparseVector(unittest.TestCase):
    ...
    def test_initialisation_by_dict_with_non_numeric_key(self):
        self.assertRaises(ValueError, sparse_vector.SparseVector, {'a': 5})

    def test_initialisation_by_list(self):
        sl = sparse_vector.SparseVector([0, 1, 2, 4])
        self.assertEquals([0, 1, 2, 4], sl)

    def test_initialisation_by_generator(self):
        gen = (x for x in (1, 2, 3))
        sl = sparse_vector.SparseVector(gen)
        self.assertEquals([1, 2, 3], sl)

    def test_initialisation_with_dtype(self):
        sl = sparse_vector.SparseVector([1, 2, 3], dtype=float)
        self.assertEquals([1.0, 2.0, 3.0], sl)

    def test_initialization_with_ndarrays(self):
        import numpy as np
        keys = np.arange(1e6, dtype=np.int)
        vals = np.arange(1e6)
        sl = sparse_vector.SparseVector((keys, vals), size=1e7, default_value=0)
        dense_expected = np.zeros(int(1e7))
        dense_expected[keys] = vals
        self.assertEquals(list(dense_expected), list(sl))
        np.testing.assert_array_almost_equal(dense_expected, sl)

    def test_random_access_write(self):
        sl = sparse_vector.SparseVector(1)
        sl[0] = 'alice'
        self.assertEquals('alice', sl[0])

    def test_random_access_read_present(self):
        sl = sparse_vector.SparseVector(2)
        sl[0] = 'brent'
        self.assertEquals('brent', sl[0])

    def test_random_access_read_absent(self):
        sl = sparse_vector.SparseVector(2, 'absent')
        sl[1] = 'clint'
        self.assertEquals('absent', sl[0])

    def test_iteration_empty(self):
        sl = sparse_vector.SparseVector(3)
        self.assertEquals([None, None, None], list(sl))

    def test_iteration_populated(self):
        sl = sparse_vector.SparseVector(5)
        sl[1], sl[3] = 'a', 'b'
    ...
    def test_string_representations_float_by_default(self):
        sl = sparse_vector.SparseVector(5, 0)
        sl[3], sl[4] = 5, 6
        self.assertEquals('[0, 0, 0, 5.0, 6.0]', repr(sl))
        self.assertEquals('[0, 0, 0, 5.0, 6.0]', str(sl))

    def test_string_representations_int(self):
        sl = sparse_vector.SparseVector(5, 0, dtype=int)
        sl[3], sl[4] = 5, 6
    ...
    def test_slice(self):
        sl = sparse_vector.SparseVector([0, 1, 2, 4], 10)
        self.assertEquals([1, 2], sl[1:3])

    def test_extended_slice(self):
        sl = sparse_vector.SparseVector([0, 1, 2, 3, 4, 5, 6, ])
    ...
    def test_slice_with_list_write(self):
        ...
        self.assertEquals([6, 2, 7, 4, 8], sl)
        import numpy as np
        np.testing.assert_array_almost_equal([6, 2, 7, 4, 8], sl)

    def test_slice_with_list_write_2(self):
        sl = sparse_vector.SparseVector(([1, 2, 4, 5], [1, 2, 3, 4]))
        ip = [0, 3, 2, 4]
        sl[ip] = [6, 7, 9, 8]
        self.assertEquals([6, 1, 9, 7, 8, 4], list(sl))
    ...
    def test_sorted(self):
        sl = sparse_vector.SparseVector({0: 1, 4: 1}, 0)
    ...
    def test_slice_removal(self):
        sl = sparse_vector.SparseVector(range(10), None)
        del sl[3:5]
        self.assertEquals([0, 1, 2, None, None, 5, 6, 7, 8, 9], sl)

    def test_append(self):
        sl = sparse_vector.SparseVector(1, 0)
        sl.append(1)
        self.assertEquals([0, 1], sl)

    def test_clone(self):
        a = sparse_vector.SparseVector([1, 2, 3])
        b = a[:]
        b.append(4)
    ...
    def test_concatenation(self):
        a = sparse_vector.SparseVector([1, 2, 3])
        b = sparse_vector.SparseVector([4, 5, 6])
        c = a + b
        self.assertEquals([1, 2, 3], a)
    ...
    def test_equality(self):
        a = sparse_vector.SparseVector([1, 2, 3])
        b = sparse_vector.SparseVector([1, 2, 3])
        self.assertTrue(a == b)
    ...
    def test_inequality_same_length(self):
        a = sparse_vector.SparseVector([1, 2, 3])
        b = sparse_vector.SparseVector([1, 0, 3])
        self.assertTrue(a != b)
        self.assertTrue(not a == b)
    ...
    def test_inequality_left_longer(self):
        a = sparse_vector.SparseVector([1, 2, 3, 4])
        b = sparse_vector.SparseVector([1, 2, 3])
        self.assertTrue(a != b)
        self.assertTrue(not (a == b))
        self.assertNotEqual(a, b)
    ...
    def test_less_than(self):
        a = sparse_vector.SparseVector([1, 2, 3, 0])
        b = sparse_vector.SparseVector([1, 2, 4, 5])
        self.assertTrue(a < b)
    ...
    def test_count_default_value(self):
        sl = sparse_vector.SparseVector(100, 1)
        sl[5] = 1
    ...
    def test_index_absent_default_value(self):
        sl = sparse_vector.SparseVector([1, 2, 3], 0)
    ...
    def test_remove_only_first_value(self):
        sl = sparse_vector.SparseVector([2, 2, 3])
        sl.remove(2)
        self.assertEquals(3, len(sl))
    ...
    def test_remove_default_value_does_nothing(self):
        sl = sparse_vector.SparseVector(4, None)
        sl.remove(None)
